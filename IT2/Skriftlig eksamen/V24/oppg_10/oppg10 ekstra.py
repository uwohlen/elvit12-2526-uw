...
ANTALL = 200 # antall firkanter i bredden og høyden
KVADRAT = 2
...
class Firkant:
    ...
    Metoder:
        ...
        tellnaboer(self,alle): teller antall levende naboer
                                alle: 2D-array med alle firkantene
        skiftfarge(self): bytter farge basert på antall levende naboer
        
    """
    def __init__(self, i:int, j:int, side:int, farge, vindusobjekt):
        self.i = i
        self.j = j
        self.x = j*side
        self.y = i*side
    ...
    def skiftfarge(self):
        if self.naboer == 3 or self.naboer == 6:
            self.farge = SVART
        elif self.naboer != 2:
            self.farge = HVIT
...
for i in range(ANTALL):
    ...
    for j in range(ANTALL):
        ...
        if ny == 0:
            ...
            tilstand = HVIT
        rute = Firkant(i, j, KVADRAT, tilstand, vindu)
        ruter[i].append(rute)
